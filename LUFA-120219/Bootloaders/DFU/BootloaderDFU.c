...
/* Counter for manual restart (FL_B) */
static uint16_t count = 0;
static uint8_t restart = 0;
static uint8_t power = 0;
static uint8_t connected = 0;

static uint8_t JumpedFromApp = 0;
...
int main(void)
{
  ...
  if(PINE & _BV(PE2) && !JumpedFromApp) /* If FR_B is NOT pressed, start application */
  {
  ...
  while (RunBootloader || WaitForExit)
  {
    ...
    if(++count > 50000)
    {
      count = 0;
      /* Toggle LED */
      if(PORTB & _BV(PB6)) PORTB &= ~_BV(PB6); else PORTB |= _BV(PB6);
      /* if FR_B is held, exit bootloader at next opportunity */
      if(PINE & _BV(PE4)) restart = 0; else restart++;

      if(chargeStatus() == 0 && restart == 0)
      {
        power++;
      }
      else
...
static char chargeStatus(void)
{
  char stat = 0;
  DDRA &= ~_BV(PA7);
  PORTA |= _BV(PA7);
  _delay_ms(1);
  if(!(PINA & _BV(PA7))) stat = 1;
  PORTA &= ~_BV(PA7);
  _delay_ms(1);
  if(PINA & _BV(PA7)) stat = 2;
  return stat;
}
...
static void SetupHardware(void)
{
  ...
  if(PORTE & _BV(PE2)) JumpedFromApp = 1; else JumpedFromApp = 0;
